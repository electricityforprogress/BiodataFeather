...
//check if button is held, potentially for reset stuff
  if(!digitalRead(buttonPin)) {
     ...
     //reset memory - chromatic scale, channel 1, wifi off, ble on
      EEPROM.write(0, 0); EEPROM.write(1, 1); EEPROM.write(2,0); EEPROM.write(3,1);
      EEPROM.commit();
         bleMIDI = 1;
         wifiMIDI = 0;
         channel = 1;
     ...
     //turn on access point and input page
     
     ledFaders[4].Set(0, 0); //does this set immediately?
     //ledFaders[4].Update(); //hopefully only needs one update
  }
  ...
  if(keybyte != 1) { //if not initialized first time - Scale,channel,wifi,bluetooth, key
    ...
    //EEPROM.write(0, 0); EEPROM.write(1, 1); EEPROM.write(2,1); EEPROM.write(3,0); EEPROM.write(4,1);
     //normal init - ble ON, wifi OFF
      EEPROM.write(0, 0); EEPROM.write(1, 1); EEPROM.write(2,0); EEPROM.write(3,1); EEPROM.write(4,1);
      EEPROM.commit();
