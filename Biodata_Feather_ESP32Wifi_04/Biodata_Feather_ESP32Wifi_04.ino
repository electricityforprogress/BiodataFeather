...
/***
1. Static IP Addressing, connect to specific session(?)
2. Reconnect after Session disconnect (?)
3. LED display of wifi status, RTP status, battery status?
  Green Wifi Connected, Red Wifi not conn, White RTP connected, Yellow RTP not conn, Blue Ble conn, blue flash ble not con
  menus
    Red - MIDI channel, Yellow - MIDI Scale, Green - WiFi on/off, Blue - Ble on/off, White - Wifi Signal level
4. Move key variables (channel, wifi cred, IP, blue/wifi mode)
5. Velocity using CC80, mapping velocity range
6. Enable CC, variable for cc number and mapping range
7. press button, display status LED and usb serial
