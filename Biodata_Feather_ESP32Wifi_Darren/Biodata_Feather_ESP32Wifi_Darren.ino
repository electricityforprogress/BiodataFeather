...
// Wifi Credentials ~~~~~~~~~~~!!!!
char ssid[] = "TelstraE5F20F"; //  your network SSID (name)
char pass[] = "kcvwn45bd"
//  Set the MIDI Chan;    // your network password (use for WPA, or use as key for WEP)
// ~~~~~~~~~~~~~!!!!nel of this node
// if running multiple nodes either use many RTP Sessions
// or change the midi Channel on each node and use a single Session
byte channel = 1; 

float batteryMin = 3.3;   
unsigned long lastBatFlash = 0;
unsigned long lowBatFlashTime = 5000;
bool lowBatToggle = false;
...
byte usbMIDI = 0; //usb MIDI connection with co-processor
...
wifiTimer = millis();
  ...
  while (WiFi.status() != WL_CONNECTED) {
    FastLED.delay(230);
    Serial.print(F("."));
    if(wifiLED) leds[2] = CRGB::Blue;
    else leds[2] = CRGB::Purple;
    wifiLED = !wifiLED; //toggle

    FastLED.show();
    if(millis()-wifiTimer > 15000) break;
  }
  Serial.println(F(""));
  if(WiFi.status() == WL_CONNECTED)  Serial.println(F("WiFi connected"));
  else Serial.println(F("Wifi unable to connect"));
