...
//*************
      ...
      if(wifiMIDI&&isConnected) AppleMIDI.sendNoteOn(value,velocity,channel);
...
//*************
      //determine serial, wifi, or ble
       ...
       if(wifiMIDI&&isConnected) {} //AppleMIDI.sendControlChange?
    }
  }
}
  ...
  for (int i = 0;i<polyphony;i++) {
    if(noteArray[i].velocity) {
      ...
      if (noteArray[i].duration <= currentMillis) {
        ...
        if(wifiMIDI && isConnected) AppleMIDI.sendNoteOff(noteArray[i].value, 0, channel);
